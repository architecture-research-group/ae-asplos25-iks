...
// Structure representing a Top-K selection mechanism
struct TopK {
	int busy_cycles;
	...
	// Function to check if a score should be inserted into the queue
...
struct PE {
	...
	// Function to run a batch of operations and return stall and useful cycles
...
struct NMA {
	...
	// Function to run one batch of operations across the PE array
	std::pair<int, int> run_one_batch(int offset, int corpus_batch_size, offload_context* offload_context) {
		int cycles = 0;
		int stall_cycles = 0;
	...
	// Function to run batches of operations across the PE array
	...
	// Function to store the number of vectors
...
struct IKS {
	...
	// Function to run the search operation across all NMAs
